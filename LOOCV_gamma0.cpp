...
cube Tmats_gamma0(int n, int p, int L, int k, mat all_nbd, mat x, mat adj_mat, vec gamma0seq, mat grand_means, cube grand_vars, vec mu0, float nu0, float phi0sq){
    ...
    for(int j=0; j<n; ++j){
      ...
      for(int u=0; u<L; ++u){
        ...
        if(i==j){
          ...
          float gamma0 = gamma0seq(u);
          float gamman = gamma0 + k;
          float df = gamman - p + 1;
          mat psi0 = eye(p, p)*(gamma0*phi0sq);
          
          mat psi = psi0 + rest;
          mat lambda = ((nun+1)/(nun*df))*psi;
